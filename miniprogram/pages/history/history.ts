import { dataManager, ToolUsageRecord as BaseToolUsageRecord } from '../../utils/dataManager'
import { formatTime } from '../../utils/index'

interface ToolUsageRecord extends BaseToolUsageRecord {
  id: string
}
...
interface UsageStats {
  ...
  weekUsage: number
  monthUsage: number
  favoriteTools: Array<{ toolName: string; count: number }>
}
...
Page({
  data: {
    ...
    usageGroups: [] as UsageGroup[],
    filteredGroups: [] as UsageGroup[],
    stats: {
      totalUsage: 0,
      totalDuration: 0,
      todayUsage: 0,
  ...
  async loadUsageHistory() {
    try {
      ...
      const baseRecords = await dataManager.getUsageHistory()
      // 添加id字段
      const records: ToolUsageRecord[] = baseRecords.map((record, index) => ({
        ...record,
        id: `${record.timestamp}_${index}`
      }))
   ...
   * 按日期分组记录
   */
  groupRecordsByDate(records: ToolUsageRecord[]): UsageGroup[] {
    const groups: Record<string, ToolUsageRecord[]> = {}
   ...
   * 计算统计信息
   */
  calculateStats(records: ToolUsageRecord[]): UsageStats {
    // const now = Date.now()
    const today = new Date()
   ...
   */
  onRecordTap(e: any) {
    const record = e.currentTarget.dataset.record as ToolUsageRecord
        ...
        if (res.confirm) {
          try {
            ...
            const filteredRecords = baseRecords.filter(r => 
              !(r.timestamp === record.timestamp && r.toolId === record.toolId)
            )
            await dataManager.saveUsageHistory(filteredRecords)
  ...
  onClearHistory() {
    wx.showModal({
      ...
      success: async (res) => {
        if (res.confirm) {
          try {
            await dataManager.clearUsageHistory()
  ...
  async onExportData() {
    try {
      const records = await dataManager.getUsageHistory()
      const exportData = {
